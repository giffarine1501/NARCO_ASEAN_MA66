<footer class="pt-4 pb-2  bg-blue ">
	<div class="row mx-0">
		<div class="container mt-50 mb-40 p-lg-0"></div>
		<div class="col-lg-3 col-md-12 col-sm-12 col-12 ">
			<div class="row">
				<div class="col-12">
					<img src="assets/img/logo-footer.png" class="logo-footer-set">
				</div>
				<div class="col-sm-12 col-md-12 col-lg-12">
					<ul class="list-unstyled">
						<div class="txtcolor-white mb-2">
							<li>เลขที่ 5 ถ.ดินแดง แขวงพญาไท เขตพญาไท กรุงเทพฯ 10400
								<br>อีเมล : aseannarco@oncb.go.th
							</li>
						</div>
					</ul>
				</div>
			</div>
		</div>

		<div class="col-lg-6 col-md-6 col-sm-12 col-12 ">
			<div class="row">
				<div class="col-12 ">
					<h5 class="txtcolor-white foot pt-2"> แผนผังเว็บไซต์ </h5>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-12 col-12 ">
					<ul class="list-unstyled">
						<div>
							<li><a class="txtcolor-white foot" href="#" title="หน้าแรก" target="_self">หน้าแรก </a>
							</li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="เกี่ยวกับเรา" target="_self">
									เกี่ยวกับเรา</a>
							</li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="ข้อมูล ปปส.อาเซียน" target="_self">
									ข้อมูล
									ปปส.อาเซียน </a></li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="โครงการสำคัญ" target="_self">
									โครงการสำคัญ</a>
							</li>
						</div>
					</ul>
				</div>

				<div class="col-lg-6 col-md-6 col-sm-12 col-12 ">
					<ul class="list-unstyled">
						<div>
							<li><a class="txtcolor-white foot" href="#" title="รายงานเฝ้าระวังยาเสพติด"
									target="_self">รายงานเฝ้าระวังยาเสพติด</a></li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="รายงาน" target="_self">รายงาน </a>
							</li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="ปฏิทินกิจกรรม"
									target="_self">ปฏิทินกิจกรรม</a></li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="ติดต่อเรา" target="_self">ติดต่อเรา
								</a></li>
						</div>
					</ul>
				</div>
			</div>
		</div>

		<div class="col-lg-3 col-md-6 col-sm-12 col-12 ">
			<div class="row">
				<div class="col-12">
					<h5 class="txtcolor-white foot pt-2"> ช่องทางติดต่อเรา </h5>
				</div>
				<div class="col-lg-12 col-md-12 col-sm-12 col-12">
					<ul class="list-unstyled">
						<div>
							<li><a class="txtcolor-white foot" href="#" title="โทรศัพท์ : 02-247-0901-19"
									target="_self">โทรศัพท์ : 02-247-0901-19 </a></li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="โทรสาร : 02-246-8526"
									target="_self">โทรสาร
									: 02-246-8526 </a></li>
						</div>
						<div>
							<li><a class="txtcolor-white foot" href="#" title="อีเมล : aseannarco@oncb.go.th"
									target="_self">อีเมล : aseannarco@oncb.go.th </a>
							</li>
						</div>
						<div>
							<li>
								<a class="txtcolor-white foot" href="#" title="facebook" target="_self"><i
										class="fab fa-facebook-square"></i></a>
								<a class="txtcolor-white foot" href="#" title="twitter" target="_self"><i
										class="fab fa-twitter-square"></i></a>
							</li>
						</div>
					</ul>
				</div>
			</div>
		</div>
	</div>
	<div class="col-lg-12 col-md-12 col-sm-12 col-12 text-center line">
		<div class="txtcolor-white foot ">&copy; สำนักงาน ป.ป.ส. อาเซียน (ASEAN Narcotics Cooperation Center) </div>
	</div>
	</div>
</footer>